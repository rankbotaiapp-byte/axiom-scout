...
function countySweepPrompt({ niche, cluster, limit }) {
  const towns = cluster.towns.join(", ");
  return `Search the live web for EVERY real ${niche.label} business in this Oregon cluster.
County: ${cluster.county} County, Oregon
Towns (all of these): ${towns}

This is a completeness sweep, not a top-N list. Return every grounded shop you can find — independents, chains, booth renters with a public shop name, and one-chair places. Skip homes with no public listing.

Query seeds (run several):
- "${niche.label} ${cluster.county} County Oregon"
- "${niche.query} ${cluster.towns[0]} OR"
${cluster.towns.slice(0, 6).map((t) => `- "${niche.label} ${t} Oregon"`).join("\n")}
- "${niche.label} near ${cluster.towns[0]} OR"

Hard bounds:
- Only Jackson County or Josephine County, Oregon.
- Do NOT include Portland, Eugene, Roseburg, Klamath Falls, or California (Crescent City, Yreka, Brookings).

For EACH business return:
- businessName
- city (the town it sits in)
- county ("Jackson" or "Josephine")
- website
- phone
- address
- oneLiner
...
Score candidate the same as a city search (0-100, cap if AI desk is evident).
...
Aim for all of them, up to ${limit}. Empty towns are fine — do not invent a shop to fill a town.`;
}


function townSearchPrompt({ niche, town, county, limit }) {
  return `One web_search. Find REAL ${niche.label} businesses in ${town}, ${county} County, Oregon.

Query: "${niche.label} ${town} Oregon" and "${niche.query} ${town} OR".

Hard bounds: only ${town} / immediately next door. Not Portland, Eugene, California.

For EACH business:
- businessName, city, county, website, phone, address, oneLiner
- bookingSignals, alreadyHasAi, aiSignals, sourceUrls
- candidate: { score 0-100, verdict prime|maybe|skip, why }

JSON only:
{ "prospects": [ { ... } ] }

Up to ${limit}. Never invent a shop or a phone.`;
}
...
export { SEARCH_SYSTEM, searchUserPrompt, countySweepPrompt, townSearchPrompt, ENRICH_SYSTEM, enrichUserPrompt };
