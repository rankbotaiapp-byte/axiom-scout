...
export function importAll(json) {
  ...
  if (!data || typeof data !== "object") throw new Error("Not a Scout library file.");
  const root = readRoot();
  root.packs = { ...root.packs, ...(data.packs || {}) };
  root.dossiers = { ...root.dossiers, ...(data.dossiers || {}) };
  if (data.last) root.last = data.last;
  writeRoot(root);
}

export function saveLast(nicheId, regionId) {
  const root = readRoot();
  root.last = { nicheId, regionId };
  writeRoot(root);
}

export function loadLast() {
  return readRoot().last || null;
}

export function isPreviewHost(host) {
  return /-[a-z0-9]+-newera1\.vercel\.app$/i.test(String(host || ""));
}
